...
function CVLook() {
  ...
  const checkEducationRowList = useMemo(() => {
    return edu.row?.length > 0 ? (
      <div className={styles.workArea}>Education</div>
    ) : null;
  }, [edu.row?.length]);

  const checkHonorsList = useMemo(() => {
    return honors.row?.length > 0 ? (
  ...
  return (
    <div className={styles.previewLook}>
              ...
              <div key={item.id}>
                <div className={styles.workArea}>
                  <div className={styles.workingDetails}>
                    ...
                    <div className={styles.sideArea}>
                      <p>
                        {"("} {format(new Date(item.jobStart), "dd/MM/yyyy")}{" "}
                        {" - "}
                        {format(new Date(item.jobEnd), "dd/MM/yyyy")} {")"}
                      </p>
                    </div>
                  </div>
      ...
      <div className={styles.rowPages}>
        ...
        {/* senc mi gri senc depqerum wrap ara kam useMemo kam useCallback,vorovhetev amen angam inqy stuguma anum ka length te che u havai re-render linum, orinaky grelem taky   */}

        {checkEducationRowList}
