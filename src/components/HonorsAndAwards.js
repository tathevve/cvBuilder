...
import { selectHonors, selectIsShown, setHonorsInfo } from '../redux/slicers/honorsSlice';
  ...
  return (
    ...
    >
      ...
      <form className={styles.sidebarInput}>
        ...
        <Button type='submit' className={styles.nextBtn} variant='outlined' onClick={(e) => addNewHonorHandler(e)} >Add</Button>

        {watch('row')?.length > 0 && hasHonor ? watch('row').map((item, index) => (
          <div key={item.id}>
            <div className={styles.oneLineInput}>
              <TextField
                ...
                onChange={(e) => setValue(`row[${index}].title`, e.target.value)}
              />
            </div>
            <div className={styles.oneLineInput}>
              <TextField
                ...
                className={styles.oneLineInputField}
                onChange={(e) => setValue(`row[${index}].subtitle`, e.target.value)}
              />
